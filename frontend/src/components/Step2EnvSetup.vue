  ...
  <div class="env-setup-panel">
            ...
            <div class="profiles-list">
              ...
              >
                ...
                <div class="profile-meta">
                  <span class="profile-profession">{{ profile.profession || t('common.label.unknownProfession') }}</span>
                </div>
                <p class="profile-bio">{{ profile.bio || t('common.label.noBio') }}</p>
      ...
      <div class="step-card" :class="{ 'active': phase === 2, 'completed': phase > 2 }">
        ...
        <div class="card-content">
          ...
          <div v-if="simulationConfig" class="config-detail-panel">
            ...
            <div class="config-block">
              <div class="config-grid">
                <div class="config-item">
                  <span class="config-item-label">{{ t('step2.simulationHours') }}</span>
                  <span class="config-item-value">{{ simulationConfig.time_config?.total_simulation_hours || '-' }} {{ locale === 'ko' ? '시간' : 'hours' }}</span>
                </div>
                <div class="config-item">
                  <span class="config-item-label">{{ t('step2.minutesPerRound') }}</span>
                  <span class="config-item-value">{{ simulationConfig.time_config?.minutes_per_round || '-' }} {{ locale === 'ko' ? '분' : 'min' }}</span>
                </div>
                <div class="config-item">
                  <span class="config-item-label">{{ t('step2.totalRounds') }}</span>
                  <span class="config-item-value">{{ Math.floor((simulationConfig.time_config?.total_simulation_hours * 60 / simulationConfig.time_config?.minutes_per_round)) || '-' }} {{ locale === 'ko' ? '라운드' : 'rounds' }}</span>
                </div>
                <div class="config-item">
            ...
            <div class="config-block">
              <div class="config-block-header">
                <span class="config-block-title">{{ t('step2.recommenderConfig') }}</span>
              </div>
              <div class="platforms-grid">
                <div v-if="simulationConfig.twitter_config" class="platform-card">
                  <div class="platform-card-header">
                    <span class="platform-name">{{ t('step2.platformOne') }}</span>
                  </div>
                  <div class="platform-params">
                    ...
                    <div class="param-row">
                      <span class="param-label">{{ t('step2.echoStrength') }}</span>
                ...
                <div v-if="simulationConfig.reddit_config" class="platform-card">
                  <div class="platform-card-header">
                    <span class="platform-name">{{ t('step2.platformTwo') }}</span>
                  </div>
                  <div class="platform-params">
                    <div class="param-row">
                      <span class="param-label">{{ t('step2.recencyWeight') }}</span>
        ...
        <div class="card-content">
          ...
          <div v-if="simulationConfig?.event_config" class="orchestration-content">
            ...
            <div class="initial-posts-section">
              <span class="box-label">{{ t('step2.initialActivation') }} ({{ simulationConfig.event_config.initial_posts.length }})</span>
          ...
          <div v-if="simulationConfig && autoGeneratedRounds" class="rounds-config-section">
            <div class="rounds-header">
              <div class="header-left">
                <span class="section-title">{{ t('step2.roundsSettings') }}</span>
                <span class="section-desc">{{ t('step2.roundsDescPrefix') }} <span class="desc-highlight">{{ simulationConfig?.time_config?.total_simulation_hours || '-' }}</span> {{ locale === 'ko' ? '시간' : 'hours' }}, {{ t('step2.roundsDescMiddle') }} <span class="desc-highlight">{{ simulationConfig?.time_config?.minutes_per_round || '-' }}</span> {{ t('step2.roundsDescSuffix') }}</span>
              </div>
            ...
            <Transition name="fade" mode="out-in">
              <div v-if="useCustomRounds" class="rounds-content custom" key="custom">
                ...
                <div class="slider-main-value">
                  ...
                  <span class="val-unit">{{ locale === 'ko' ? '라운드' : 'rounds' }}</span>
                </div>
                <div class="slider-meta-info">
                    <span>{{ t('step2.estimatedFor100Prefix') }} {{ Math.round(customMaxRounds * 0.6) }} {{ t('step2.estimatedFor100Suffix') }}</span>
                  </div>
                </div>
                  ...
                  <div class="auto-content">
                    <div class="auto-meta-row">
                      <span class="duration-badge">
                        ...
                        {{ t('step2.estimatedFor100Prefix') }} {{ Math.round(autoGeneratedRounds * 0.6) }} {{ t('step2.estimatedFor100Suffix') }}
                      </span>
                    </div>
                    <div class="auto-desc">
        ...
        <div class="modal-body">
          ...
          <div class="modal-info-grid">
            ...
            <div class="info-item">
              ...
              <span class="info-value">{{ getGenderLabel(selectedProfile.gender) }}</span>
            </div>
            <div class="info-item">
              <span class="info-label">{{ t('step2.region') }}</span>
          ...
          <div class="modal-section">
            ...
            <p class="section-bio">{{ selectedProfile.bio || t('common.label.noBio') }}</p>
          </div>
    ...
    <div class="system-logs">
      <div class="log-header">
        ...
        <span class="log-id">{{ simulationId || t('common.label.noSimulation') }}</span>
      </div>
      <div class="log-content" ref="logContent">
        <div class="log-line" v-for="(log, idx) in systemLogs" :key="idx">
          <span class="log-time">{{ log.time }}</span>
          <span class="log-msg">{{ rt(log.msg) }}</span>
        </div>
      </div>
    </div>
  </div>
...
const props = defineProps({
  simulationId: String,  // 상위 구성요소에서 전달됨
  projectData: Object,
  graphData: Object,
  systemLogs: Array
})
...
// State
const phase = ref(0) // 0: 초기화, 1: 캐릭터 생성, 2: 구성 생성, 3: 마치다
...
// 시뮬레이션 라운드 구성
const useCustomRounds = ref(false) // 기본적으로 자동 구성 라운드 사용
const customMaxRounds = ref(40)   // 기본적으로 40라운드가 권장됩니다.

// Watch stage to update phase
watch(currentStage, (newStage) => {
  if (newStage === '에이전트 페르소나 생성' || newStage === 'generating_profiles') {
    phase.value = 1
  } else if (newStage === '시뮬레이션 구성 생성' || newStage === 'generating_config') {
    phase.value = 2
...
// 구성에서 자동으로 생성된 라운드 계산(하드코딩된 기본값을 사용하지 않음)
const autoGeneratedRounds = computed(() => {
  if (!simulationConfig.value?.time_config) {
    return null // 구성이 생성되지 않은 경우 null을 반환합니다.
  }
  const totalHours = simulationConfig.value.time_config.total_simulation_hours
  const minutesPerRound = simulationConfig.value.time_config.minutes_per_round
  if (!totalHours || !minutesPerRound) {
    return null // 구성 데이터가 불완전한 경우 null을 반환합니다.
  }
  const calculatedRounds = Math.floor((totalHours * 60) / minutesPerRound)
  // 비정상적인 슬라이더 범위를 방지하려면 최대 라운드 수가 40(권장 값) 이상인지 확인하세요.
...
const getGenderLabel = (gender) => {
  if (gender === 'male') return t('step2.male')
  if (gender === 'female') return t('step2.female')
  if (gender === 'other') return t('step2.other')
  return gender || '-'
}
...
const handleStartSimulation = () => {
  // 상위 구성요소에 전달된 구성 매개변수
  const params = {}

  if (useCustomRounds.value) {
    // 사용자 정의 라운드 수, 최대 통과_rounds 매개변수
    params.maxRounds = customMaxRounds.value
    addLog(`시뮬레이션을 시작하고 라운드 수를 맞춤 설정하세요: ${customMaxRounds.value} 바퀴`)
  } else {
    // 사용자는 자동 생성된 라운드 수를 유지하고 최대 라운드 수를 통과하지 않기로 선택합니다._rounds 매개변수
    addLog(`라운드 수 자동 구성을 사용하여 시뮬레이션 시작: ${autoGeneratedRounds.value} 바퀴`)
  }
...
const startPrepareSimulation = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // 예상되는 총 에이전트 수를 즉시 설정합니다(준비 인터페이스의 반환 값에서 가져옴).
      if (res.data.expected_entities_count) {
        expectedTotal.value = res.data.expected_entities_count
        addLog(`Zep 스펙트럼에서 읽기 ${res.data.expected_entities_count} 엔터티`)
        if (res.data.entity_types && res.data.entity_types.length > 0) {
          addLog(`  └─ 엔터티 유형: ${res.data.entity_types.join(', ')}`)
        }
      }
...
const fetchProfilesRealtime = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // 출력 프로필은 진행 로그를 생성합니다(수량이 변경되는 경우에만).
      ...
      if (currentCount > 0 && currentCount !== lastLoggedProfileCount) {
        ...
        if (currentCount === 1) {
          addLog(`에이전트 프로필 생성 시작...`)
        }
        addLog(`→ Agent성격 ${currentCount}/${total}: ${profileName} (${latestProfile?.profession || '직업불명'})`)

        // 모든 생성이 완료되면
        if (expectedTotal.value && currentCount >= expectedTotal.value) {
          addLog(`✓ 모두 ${currentCount} 상담원 프로필 생성 완료`)
        }
      }
    }
  } catch (err) {
    console.warn('프로필을 가져오지 못했습니다.:', err)
  }
}
...
const fetchConfigRealtime = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // 출력 구성 생성 단계 로그(중복 방지를 위해)
      if (data.generation_stage && data.generation_stage !== lastLoggedConfigStage) {
        lastLoggedConfigStage = data.generation_stage
        if (data.generation_stage === 'generating_profiles') {
          addLog('에이전트 프로필 구성 생성 중...')
      ...
      if (data.config_generated && data.config) {
        ...
        addLog('✓ 시뮬레이션 구성 생성 완료')

        // 자세한 구성 요약 표시
        if (data.summary) {
          addLog(`  ├─ Agent수량: ${data.summary.total_agents}개인`)
          addLog(`  ├─ 시뮬레이션 기간: ${data.summary.simulation_hours}시간`)
          addLog(`  ├─ 초기 게시물: ${data.summary.initial_posts_count}조각`)
          addLog(`  ├─ 뜨거운 주제: ${data.summary.hot_topics_count}개인`)
          addLog(`  └─ 플랫폼 구성: Twitter ${data.summary.has_twitter_config ? '✓' : '✗'}, Reddit ${data.summary.has_reddit_config ? '✓' : '✗'}`)
        }

        // 시간 구성 세부정보 표시
        if (data.config.time_config) {
          const tc = data.config.time_config
          addLog(`시간 구성: 매 라운드${tc.minutes_per_round}분, 흔한${Math.floor((tc.total_simulation_hours * 60) / tc.minutes_per_round)}바퀴`)
        }
    ...
    console.warn('구성을 가져오지 못했습니다.:', err)
  }
}
  ...
  try {
    ...
    if (res.success && res.data) {
      if (res.data.config_generated && res.data.config) {
        ...
        // 자세한 구성 요약 표시
        if (res.data.summary) {
          addLog(`  ├─ Agent수량: ${res.data.summary.total_agents}개인`)
          addLog(`  ├─ 시뮬레이션 기간: ${res.data.summary.simulation_hours}시간`)
          addLog(`  └─ 초기 게시물: ${res.data.summary.initial_posts_count}조각`)
        }
    ...
    addLog(`구성을 로드하지 못했습니다.: ${err.message}`)
    emit('update-status', 'error')
  }
}
