...
# Track whether cleanup handlers have been registered
...
class SimulationRunState:
    ...
    # Per-platform round counters and simulated time (for dual-platform parallel display)
    ...
    # Platform completion state (detected from simulation_end events in actions.jsonl)
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            # Per-platform round counters and simulated time
...
class SimulationRunner:
    ...
    3. Provide real-time status query APIs
    ...
    def start_simulation(
        ...
        Args:
            ...
            enable_graph_memory_update: Whether to dynamically write agent activity back into the Zep graph
        ...
        # If max rounds is specified, cap the total rounds
        ...
        # If graph memory updates are enabled, create an updater
        ...
        try:
            # Build the run command using absolute paths
            # New log layout:
            ...
            # If max rounds is specified, add it to the command line
            if max_rounds is not None and max_rounds > 0:
                cmd.extend(["--max-rounds", str(max_rounds)])
            
            # Create the main log file to avoid stdout/stderr pipe buffering from blocking the process
            main_log_path = os.path.join(sim_dir, "simulation.log")
            main_log_file = open(main_log_path, 'w', encoding='utf-8')
            
            # Set child-process environment variables to ensure UTF-8 encoding on Windows
            # This fixes issues where third-party libraries (such as OASIS) read files without an explicit encoding
            env = os.environ.copy()
            env['PYTHONUTF8'] = '1'  # Python 3.7+ support so all open() calls default to UTF-8
            env['PYTHONIOENCODING'] = 'utf-8'  # Ensure stdout/stderr use UTF-8
            
            # Set the working directory to the simulation directory (databases and other files are created there)
    ...
    def _monitor_simulation(cls, simulation_id: str):
        ...
        try:
            ...
            # After the process exits, read the logs one final time
            if os.path.exists(twitter_actions_log):
            ...
            if cls._graph_memory_enabled.get(simulation_id, False):
                try:
                    ...
                    logger.error(f"Failed to stop the graph memory updater: {e}")
                        ...
                        try:
                            ...
                            if "event_type" in action_data:
                                ...
                                if event_type == "simulation_end":
                                    ...
                                    # If only one platform is running, check that platform only
                                ...
                                # Update round info (from round_end events)
                                    ...
                                    # Update per-platform rounds and times
                                    if platform == "twitter":
                                    ...
                                    # Use the max round across both platforms as the overall round
                                    if round_num > state.current_round:
                                        state.current_round = round_num
                                    # Use the max simulated time across both platforms as the overall time
                            ...
                            # If graph memory updates are enabled, send activities to Zep
                            if graph_updater:
    ...
    def _check_all_platforms_completed(cls, state: SimulationRunState) -> bool:
        ...
        Determine whether a platform is enabled by checking whether its actions.jsonl file exists
        ...
        # Return False if a platform is enabled but not completed
    ...
    def _terminate_process(cls, process: subprocess.Popen, simulation_id: str, timeout: int = 10):
        """
        Terminate a process and its child processes across platforms
        ...
        if IS_WINDOWS:
            ...
            # Unix: Use process groups for termination
            ...
            try:
                ...
                # If it still has not exited after the timeout, send SIGKILL
        ...
        with open(file_path, 'r', encoding='utf-8') as f:
            for line in f:
                ...
                try:
                    ...
                    # Skip non-action records (such as simulation_start, round_start, and round_end events)
                    ...
                    # Determine the platform: use the record's platform first, otherwise use the default platform
    ...
    def get_all_actions(
        ...
        # Read the Twitter action file (auto-set platform=twitter from the file path)
        twitter_actions_log = os.path.join(sim_dir, "twitter", "actions.jsonl")
        ...
        # Read the Reddit action file (auto-set platform=reddit from the file path)
        reddit_actions_log = os.path.join(sim_dir, "reddit", "actions.jsonl")
        ...
        # If per-platform files do not exist, try the legacy single-file format
        if not actions:
            actions_log = os.path.join(sim_dir, "actions.jsonl")
            actions = cls._read_actions_from_file(
                actions_log,
                default_platform=None,  # The legacy-format file should contain a platform field
                platform_filter=platform,
                agent_id=agent_id,
                round_num=round_num
            )
    ...
    def cleanup_simulation_logs(cls, simulation_id: str) -> Dict[str, Any]:
        """
        Clean up simulation runtime logs (used for forced restarts)
        ...
        Note: config files (simulation_config.json) and profile files are not deleted
        ...
        for filename in files_to_delete:
            ...
            if os.path.exists(file_path):
                try:
                    ...
                    errors.append(f"Failed to delete {filename} failed: {str(e)}")
            ...
            if os.path.exists(dir_path):
                ...
                if os.path.exists(actions_file):
                    try:
                        ...
                        errors.append(f"Failed to delete {dir_name}/actions.jsonl failed: {str(e)}")
        
        # Clear in-memory runtime state
        if simulation_id in cls._run_states:
            del cls._run_states[simulation_id]
        
        logger.info(f"Finished cleaning simulation logs: {simulation_id}, Delete files: {cleaned_files}")
    ...
    # Guard flag to prevent duplicate cleanup
    _cleanup_done = False
    ...
    def cleanup_all_simulations(cls):
        ...
        # Check whether there is anything to clean up (avoid useless logs when there are no processes)
        ...
        # Copy the dict to avoid mutating it during iteration
        ...
        for simulation_id, process in processes:
            try:
                if process.poll() is None:  # the process is still running
                    ...
                    # Also update state.json to set the status to stopped
                    try:
                        sim_dir = os.path.join(cls.RUN_STATE_DIR, simulation_id)
        ...
        logger.info("Finished cleaning simulation processes")
    ...
    def register_cleanup(cls):
        ...
        Call this when the Flask app starts so all simulation processes are cleaned up on server shutdown
        """
        ...
        # In Flask debug mode, register cleanup only in the reloader child process (the process actually running the app)
        # WERKZEUG_RUN_MAIN=true indicates the reloader child process
        # In non-debug mode this env var is absent, and registration is still needed
        ...
        # SIGHUP Exists only on Unix systems (macOS/Linux), not Windows
    ...
    def check_env_alive(cls, simulation_id: str) -> bool:
        ...
        Returns:
            True means the environment is alive; False means it has shut down
        """
        sim_dir = os.path.join(cls.RUN_STATE_DIR, simulation_id)
        if not os.path.exists(sim_dir):
            return False
    ...
    def get_env_status_detail(cls, simulation_id: str) -> Dict[str, Any]:
        ...
        Returns:
            Status detail dict containing status, twitter_available, reddit_available, and timestamp
        """
        sim_dir = os.path.join(cls.RUN_STATE_DIR, simulation_id)
    ...
    def interview_agent(
        ...
        if not ipc_client.check_env_alive():
            raise ValueError(f"The simulation environment is not running or has closed; cannot execute Interview: {simulation_id}")
    ...
    def interview_agents_batch(
        ...
        Args:
            ...
            platform: Default platform (optional; overridden by each interview item's platform)
                - "twitter": Interview only the Twitter platform by default
                - "reddit": Interview only the Reddit platform by default
    ...
    def interview_all_agents(
        ...
        if not agent_configs:
            raise ValueError(f"No agents found in the simulation config: {simulation_id}")
    ...
    def close_simulation_env(
        ...
        Send a close-environment command so the simulation exits wait-for-command mode gracefully
        ...
        try:
            ...
            return {
                "success": True,
                "message": "Close-environment command sent (timed out waiting for a response; the environment may already be closing)"
            }
    ...
    def get_interview_history(
        ...
        """
        Get Interview history (from the database)
        ...
        Args:
            ...
            agent_id: Specific Agent ID (optional; return only that agent's history)
            limit: Maximum number of rows to return per platform
