...
【输出语言要求】
- 报告标题、摘要、章节标题、章节描述必须全部使用 {report_language}
- 不要输出中文，除非 {report_language} 本身就是中文（本项目中不使用中文输出）
...
3. 【语言一致性 - 引用内容必须翻译为报告语言】
   ...
   - 本次报告的输出语言必须全部使用 {report_language}
   - 当你引用工具返回的英文或混合语言内容时，必须将其翻译为流畅的 {report_language} 后再写入报告
   - 翻译时保持原意不变，确保表述自然通顺
